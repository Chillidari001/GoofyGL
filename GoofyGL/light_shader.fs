...
//vec3 CalculateSpotLight(SpotLight spot_light, vec3 normal, vec3 frag_pos, vec3 view_dir);
...
//vec3 CalculateSpotLight(spot_light, norm, frag_pos, view_dir)
//{
    //spotlight stuff, do later
//}
...
void OldLightingRef()
{
    ...
    //frag_color = vec4(light.ambient * texture(material.diffuse, tex_coords).rgb,1.0f);
}